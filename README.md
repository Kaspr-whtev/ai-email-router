...
1. W wybranym folderze należy skolować repozytorium:
   ```
   git clone https://github.com/Kaspr-whtev/ai-email-router.git
   ```
3. Upewnić się że silnik Dockera jest włączony (Docker Engine running)
4. Wewnątrz głównego folderu sklonowanego repozytorium uruchomić:
   ```
   docker compose up -d
   ```
